...
def resolve_pending(memory_dir) -> tuple[Path, dict]:
    ...
    pending = (
        raw_pending.resolve() if raw_pending.is_absolute() else (Path.cwd() / raw_pending).resolve()
    )
    expected = root / str(cycle)
    if pending != expected or pending.parent != root:
        raise ValueError(
            f"pending pointer/dir mismatch: cycle {cycle} must resolve to {expected}, not {pending}"
        )
