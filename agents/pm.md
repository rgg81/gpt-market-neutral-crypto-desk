...
- [c55; manifest-bound seat-quality and liveness recalibration] Retire c47's automatic two-specialist conviction threshold: c52 and c53 contain explicit PM gate declarations, and the sealed state-liveness recurrence establishes that the same calibration excluded every alpha seat for three consecutive Books despite complete specialist coverage and non-flat technical candidates. Let a candidate with one strong current technical case grounded in beta-adjusted relative price reach full PM judgment; this is eligibility for review, never a mandate to select or deploy. For every discretionary new, flipped, or same-side increased non-BTC slice, use all complete specialist evidence, require a documented positive price-edge case after size-aware entry friction, compare it with cash/no-change and the best eligible replacement, explicitly discount contrary evidence, and state an objective invalidation. Strong side-opposed 72h/168h momentum remains controlling; carry may lead only in verified chop. Size from expected edge and risk, not conviction. retire_if: realized_edge_ex_funding_frac >= 0 over 3 consecutive manifest-bound scored deployed decisions.
<!-- REFLECTOR:END -->
